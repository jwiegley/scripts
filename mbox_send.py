...
option_list = [
    ...
    make_option('--smtpUser', dest='smtpUser', default=defUser,
                help="The username for the SMTP server."),

    make_option('--smtpPassword', dest='smtpPassword', default=defPassword,
...
def main(args):
    ...
    for mboxfile in arguments:
        ...
        while (current < totalInMbox and
            ...
            stmpServer = options.smtpHost + ":" + str(options.smtpPort)
            print "SMTP Server: " + stmpServer

            smtp = smtplib.SMTP(stmpServer)
            #smtpPassword = getpass.getpass()
            if options.smtpPassword: # use TSL
                #smtp.ehlo()
                smtp.starttls()
                #smtp.ehlo()
                print "USER: " + options.smtpUser
                #print "PASS: " + smtpPassword
                smtp.login(options.smtpUser, options.smtpPassword)
